<!--
  ...
  Piaf / Pmax estimate update live in the chart, then save the choice.
...
<script lang="ts">
	...
	//
	// A session "needs Piaf backfill" when no record has y ≤ 0. Adding a
	// (force, 0) at force < records[0].x closes that gap and makes the
	// bracket-midpoint branch fire.
	//
	// A session "needs Pmax backfill" when no record has y ≥ 100 — i.e. the
	...
	function needsPiaf(records: readonly PressureRecord[]): boolean {
		if (records.length === 0) return false;
		return !records.some(([, y]) => y <= 0);
	}
	function needsPmax(records: readonly PressureRecord[]): boolean {
		if (records.length === 0) return false;
	...
	let candidates = $derived<SessionView[]>(
		data.sessions
			.filter((s) => needsPiaf(s.Records) || needsPmax(s.Records))
			.map((s) => ({
				_id: s._id,
				...
				records: s.Records,
				needsPiaf: needsPiaf(s.Records),
				needsPmax: needsPmax(s.Records),
			}))
	...
	let totalPiafNeeded = $derived(candidates.filter((s) => s.needsPiaf).length);
	let totalPmaxNeeded = $derived(candidates.filter((s) => s.needsPmax).length);
	...
	// prependPiafForce / appendPmaxForce the user committed via "Save".
	type SavedEdit = {
		_id: string;
		brand: string;
		inventoryId: string;
		...
		entityId: string;
		penLabel: string;
		prependPiafForce?: number;
		appendPmaxForce?: number;
	};
	let saved = $state<Record<string, SavedEdit>>({});

	function defaultPiafForce(records: readonly PressureRecord[]): number {
	...
	function defaultPmaxForce(records: readonly PressureRecord[]): number {
	...
	let piafForce = $state<number>(0);
	let pmaxForce = $state<number>(0);
	...
	$effect(() => {
		...
		piafForce = prev?.prependPiafForce ?? defaultPiafForce(s.records);
		pmaxForce = prev?.appendPmaxForce ?? defaultPmaxForce(s.records);
	});

	// Slider ranges. Piaf goes from 0 up to the first measured force; Pmax
	...
	// Records with the user's proposed extras spliced in. Piaf prepends
	// (force, 0); Pmax appends (force, 100). Records remain sorted because
	// of the slider ranges above (piafForce ≤ first existing x; pmaxForce ≥
	// last existing x).
	let augmentedRecords = $derived.by<readonly PressureRecord[]>(() => {
		if (!current) return [];
		const recs: PressureRecord[] = [...current.records];
		if (current.needsPiaf) recs.unshift([piafForce, 0]);
		if (current.needsPmax) recs.push([pmaxForce, 100]);
		return recs;
	});

	// Bracket-midpoint estimates over the augmented records. Computed
	// locally (not via estimatePiaf/estimatePmax) so the readout reflects
	...
	function bracketPiaf(records: readonly PressureRecord[]): number | null {
	...
	function bracketPmax(records: readonly PressureRecord[]): number | null {
	...
	let estPiaf = $derived(bracketPiaf(augmentedRecords));
	let estPmax = $derived(bracketPmax(augmentedRecords));
	...
	function saveCurrent() {
		...
		if (s.needsPiaf) edit.prependPiafForce = piafForce;
		if (s.needsPmax) edit.appendPmaxForce = pmaxForce;
		saved[s._id] = edit;
	}
...
<div class="page">
	<header>
		...
		<p class="lead">
			...
			a plausible force value and watch the bracket-midpoint Piaf / Pmax estimate update. Save your
		...
		<p class="meta">
			...
			{totalPiafNeeded} need a 0% endpoint, {totalPmaxNeeded} need a 100% endpoint.
		...
		<section class="panels">
			<div class="panel">
				...
				{#if current.needsPiaf}
					...
					<div class="slider-row">
						<label>
							<span class="slider-label">Piaf (force at 0%)</span>
							<input type="range" min={r.min} max={r.max} step={r.step} bind:value={piafForce} />
							<input
								type="number"
								min={r.min}
								max={r.max}
								step={r.step}
								bind:value={piafForce}
								class="num-input"
							/>
						</label>
						<div class="slider-meta">
							...
							<span class="est">
								bracket Piaf = <strong>{estPiaf === null ? '—' : estPiaf.toFixed(2)}</strong> gf
							</span>
						</div>
					</div>
				{:else}
					<div class="slider-row inactive">
						<span class="slider-label">
							Piaf: already bracketed ({current.records.find(([, y]) => y <= 0)?.join(', ')})
						</span>
					</div>
				{/if}
			</div>
			...
			<div class="panel">
				...
				{#if current.needsPmax}
					...
					<div class="slider-row">
						<label>
							<span class="slider-label">Pmax (force at 100%)</span>
							<input type="range" min={r.min} max={r.max} step={r.step} bind:value={pmaxForce} />
							<input
								type="number"
								min={r.min}
								max={r.max}
								step={r.step}
								bind:value={pmaxForce}
								class="num-input"
							/>
						</label>
						<div class="slider-meta">
							...
							<span class="est">
								bracket Pmax = <strong>{estPmax === null ? '—' : estPmax.toFixed(2)}</strong> gf
							</span>
						</div>
					</div>
				{:else}
					<div class="slider-row inactive">
						<span class="slider-label">Pmax: already saturated or bracketed</span>
					</div>
				{/if}
			</div>
		</section>
			...
			<ul class="saved-summary">
				{#each savedList as e (e._id)}
					<li>
						...
						{#if e.prependPiafForce !== undefined}Piaf=<strong>{e.prependPiafForce}</strong>{/if}
						{#if e.prependPiafForce !== undefined && e.appendPmaxForce !== undefined}
							·
						{/if}
						{#if e.appendPmaxForce !== undefined}Pmax=<strong>{e.appendPmaxForce}</strong>{/if}
					</li>
				{/each}
			</ul>
		{/if}
