...
<ChromeLayout subNavTabs={dataTabs}>
	...
	<details class="ref">
		...
		<ul class="ops">
			...
			<li>
				<code>first</code> / <code>last</code> — raw value of the first / last item per group in input
				order (includes empties).
			</li>
		...
		<p>
			...
			columns.
			<code>.distinct(field)</code> and its synonym <code>.values(field)</code> return a sorted array
			of distinct non-empty values.
		</p>
		<p>
		...
		<p>
			<code>.derive(cols)</code> takes an object whose keys are new column names and whose values
			are functions of the row. Computed columns are usable by downstream
		...
		<p>
			...
			into matched rows. <code>.semijoin(other, leftKey, rightKey)</code> keeps left rows that have
			a match without merging — row shape stays as the left side. <code>.antijoin()</code> is the
		...
		<p>
			...
			SQL <code>ORDER BY</code>); chained <code>.sort()</code> calls compose via stable sort but
			with the <em>last</em> call as primary — prefer the array form for multi-key sorts.
		</p>
		<p>
			<code>.filterIn(field, values)</code> / <code>.filterNotIn()</code> are sugar over an OR tree
			of
			<code>==</code>
			...
			and
			<code>.collectBy(field)</code> are async terminals returning a <code>Record</code> keyed by the
			field value (single row vs array of rows per key).
		</p>
		<p>
		...
		<ul class="ops">
			...
			<li>
				<code>'&gt;'</code>, <code>'&gt;='</code>, <code>'&lt;'</code>, <code>'&lt;='</code> — numeric
				compare (blanks excluded)
			</li>
		</ul>
		<p>
