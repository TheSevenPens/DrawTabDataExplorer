...
function ensureLink({ name, target, kind }) {
	...
	if (!fs.existsSync(DATA_DIR)) {
		console.warn(
			`[setup-static] data-repo/data not found — run 'git submodule update --init --recursive' first; skipping`,
		);
	...
	if (stat) {
		if (stat.isSymbolicLink()) {
			...
			console.warn(
				`[setup-static] ${name}: real ${stat.isDirectory() ? 'directory' : 'file'} present, not a symlink — leaving alone`,
			);
