<script lang="ts">
	...
	function remove(index: number) { sorts.splice(index, 1); onchange(); contextMenu = null; }
	...
	function onDragEnd() {
		if (!droppedInside && dragIndex !== null) { sorts.splice(dragIndex, 1); onchange(); }
		dragIndex = null;
		dragOverIndex = null;
	}
</script>
...
<style>
	...
	.sort-btn.has-active:hover, .sort-btn.has-active.open { background: #eff6ff; }

	.sort-summary { font-size: 11px; color: #2563eb; font-weight: 500; }
	...
	.sort-pill { background: var(--pill-sort-bg); border: 1px solid var(--pill-sort-border); cursor: grab; gap: 4px; transition: opacity 0.15s, border-color 0.15s; }
